get_customer_segments <-
  function(k = 4, seed = 123) {
    ...
    customer_trends_tbl <-  bike_orderlines_tbl %>% 
      ...
      select(bikeshop_name, price, model, category_1, category_2, frame_material,
             quantity) %>% 
...
plot_customer_heatmap <-
  function(interactive = TRUE) {
    ...
    g <-  pct_sales_by_customer_tbl %>% 
      ...
      geom_text(aes(label = scales::percent(pct,accuracy = 0.1), text = label_text),
                size = 3) +
      ...
      theme(
        ...
        plot.title = element_text(face = "bold", vjust = 10),
...
plot_customer_behavior_by_cluster <-
  function(top_n_products = 10, 
    ...
    top_n_tbl <-   bike_orderlines_tbl %>% 
      ...
      group_by_at(vars(bikeshop_name:price)) %>% 
      summarise(total_qty = sum(quantity)) %>% 
      ungroup() %>% 
      ...
      left_join(
        combined_tbl %>% select(bikeshop_name, .cluster),
        by = "bikeshop_name"
      ) %>% 
    ...
    g <-  top_n_tbl %>% 
      ...
      geom_violin() + 
      geom_jitter(aes(text = label_text),width = .2, alpha = 0.5) + 
